...
%0.6448 0.3763 0.1909 0.4283 0.4820 0.1206 0.5895 0.2262 0.3846 0.5830];
v2(:,1)=[0.2518 0.2904 0.6171 0.2653 0.8244 0.9827 0.7302 0.3439 0.5841
0.1078];
%0.9063 0.8797 0.8178 0.2607 0.5944 0.0225 0.4253 0.3127 0.1615 0.1788];
...
for j=1:p
f(j,1)= 100*( (x1(j,1))^2 - x2(j,1) )^2 + (1- x1(j,1))^2 ; kount=kount+1;
end
%Initial personal besst values
x1p=x1(:,1);
x2p=x2(:,1);
%for Initial Global best values updation fmin=min(f(:,1));
for k=1:p
if f(k,1)==fmin
gb=k;
else
...
lim=14;
for iter=1:it
disp(sprintf('This is %d no. of iteration',iter))
if iter==lim+1
fold=f(:,1:lim);
x1old(:,1:lim)=x1(:,1:lim);
x2old(:,1:lim)=x2(:,1:lim);
x1new=zeros(p,it-lim);
x2new=zeros(p,it-lim);
for par=1:p
x1new(par,1)=(sum(x1(par,1:i).*(f(par,1:i)).^-
1))/sum(f(par,1:i).^-1);
x2new(par,1)=(sum(x2(par,1:i).*(f(par,1:i)).^-
1))/sum(f(par,1:i).^-1);
end
x1=x1new;
x2=x2new;
...
for j=1:p
fp(j)= 100*( (x1p(j))^2 - x2p(j) )^2 + (1- x1p(j))^2 ; kount=kount+1;
end
for k=1:p
if f(k,i)< fp(k)
x1p(k)=x1(k,i);
...
',minf,x1g(1),x2g(1)))
kount+1
x1all(1:p,1:lim)=x1old;
x1all(1:p,15:it)=x1;
x2all(1:p,1:lim)=x2old;
x2all(1:p,15:it)=x2;
fall(1:p,1:lim)=fold;
fall(1:p,15:it)=f;
for m=1:p
figure(m)
subplot(2,1,1)
plot(x1all(m,1:iter),x2all(m,1:iter))
hold on
plot(x1all(m,1),x2all(m,1),'r*')
hold on
plot(x1all(m,i),x2all(m,i),'o')
hold on
subplot(2,1,2)
plot(1:iter,fall(m,1:iter))
hold on
end
