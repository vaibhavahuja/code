        clc
        clear
        disp(' we have to minimize f = 100(x1^2-x2)^2+(1-x1)^2 i.e. rosenbrockfunction')
        ...
        w=0.6
        for j=1:p
        v1(j,(i+1)) = w*v1(j,i) + rp*cp*(x1p(j)-x1(j,i)) + rg*cg*(x1g(j)-
        x1(j,i));
        v2(j,(i+1)) = w*v2(j,i) + rp*cp*(x2p(j)-x2(j,i)) + rg*cg*(x2g(j)-
        x2(j,i));
        ...
        figure(m)
        subplot(2,1,1)
        plot(x1(m,1:i),x2(m,1:i))
        hold on
        plot(x1(m,1),x2(m,1),'r*')
        hold on
        plot(x1(m,i),x2(m,i),'o')
        hold on
        subplot(2,1,2)
        plot(1:i,f(m,1:i))
        hold on
